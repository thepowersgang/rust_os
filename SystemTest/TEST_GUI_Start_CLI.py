...
# Mapping from qemu's key names to the rust_os logging names
KEYNAME_MAP = {
    'meta_l': "LeftGui",
    'ret': "Return",
    'down': "DownArrow",
...
def _keypress(instance: TestInstance.Instance, key: "list[str]|str", name: str, idle=True):
    def assert_ev(ev: str, key: str):
        keyname = KEYNAME_MAP[key]
        test_assert(
            f"{name} {key} {ev} timeout",
            instance.wait_for_line(
                    r"\[syscalls\] - USER> (Window|Menu)::handle_event\(ev=Key{ev}\({keyname}\)\)".format(ev=ev,keyname=keyname),
                    timeout=timeout
                    )
            )
    timeout = 2.0
    if isinstance(key, (list,)):
        ...
        assert_ev("Up"  , key)
        if idle:
            test_assert(f"{name} {key} release idle", instance.wait_for_idle(timeout=5))

def _gui_rerender(instance: TestInstance.Instance, name: str, window_names: "list[str]"):
    for win in window_names:
        line = instance.wait_for_line(r"\[gui::windows\] - L\d+: WindowGroup::redraw: \d+ '([^']+)' dirty", timeout=10)
        test_assert("%s - Timeout waiting for '%s'" % (name, win,), line)
        assert line is not False
        if line.group(1) != win:
            raise TestFail("%s - Unexpected window rendered - \"%s\" != exp \"%s\"" % (name, line.group(1), win,))
    
def _startapp(instance: TestInstance.Instance, path: str, timeout=5):
    instance.wait_startapp(path, timeout)
    
def _mouseto(instance: TestInstance.Instance, name: str, x:int,y:int):
    instance.mouse_to(x,y)
    test_assert(name+" mouse(%i,%i) reached" % (x,y,), instance.wait_for_line(r"windows\] - CursorPos::update - \(%i,%i\)" % (x,y), timeout=3))
...
def _mouseclick(instance: TestInstance.Instance, name: str, btn: int):
    instance.mouse_press(btn) 
    test_assert("%s mouse(%i down) event" % (name, btn,), instance.wait_for_line(r"syscalls\] - USER> Window::handle_event\(ev=MouseDown\(\d+, \d+, %i\)\)" % (btn-1,), timeout=3))
    test_assert(name+" mouse(%i down) idle" % (btn,), instance.wait_for_idle(timeout=5))
    time.sleep(0.5)
    instance.mouse_release(btn) 
    test_assert("%s mouse(%i up) event" % (name, btn,), instance.wait_for_line(r"syscalls\] - USER> Window::handle_event\(ev=MouseUp\(\d+, \d+, %i\)\)" % (btn-1,), timeout=3))
...
def test(instance: TestInstance.Instance):
    ...
    if False:
        ...
        test_assert("CLI window render", instance.wait_for_line(r"\[gui::windows\] - L\d+: WindowGroup::redraw: \d+ 'Console'", timeout=5))
        ...
        test_assert("`exit` return release timeout", instance.wait_for_line(r"\[syscalls\] - USER> Window::handle_event\(ev=KeyUp\(Return\)\)", timeout=1)) # Release
        test_assert("`exit` reap", instance.wait_for_line(r"Reaping thread 0x[0-9a-f]+\(\d+ /sysroot/bin/simple_console#1\)", timeout=2))
        ...
        test_assert("final render", instance.wait_for_line(r"WindowGroup::redraw: render_order=\[\(1, \[\]\), \(4, \[\(0,20 \+ \d+x\d+\)\]\), \(5, \[\(0,0 \+ \d+x20\)\]\)\]", timeout=5))
    ...
    if False:
        ...
        test_assert("FileBrowser window render", instance.wait_for_line(r"\[gui::windows\] - L\d+: WindowGroup::redraw: \d+ 'File browser'", timeout=5))
        ...
        # Close using alt-f4
        #_keypress(instance, ['alt', 'f4'])
        instance.type_combo(['alt', 'f4'])
        test_assert("Close FileBrowser alt press timeout", instance.wait_for_line(r"\[syscalls\] - USER> Window::handle_event\(ev=KeyDown\(LeftAlt\)\)", timeout=1))
        test_assert("Close FileBrowser f4 press timeout" , instance.wait_for_line(r"\[syscalls\] - USER> Window::handle_event\(ev=KeyDown\(F4\)\)", timeout=1))
        test_assert("Close FileBrowser f4 release timeout", instance.wait_for_line(r"\[syscalls\] - USER> Window::handle_event\(ev=KeyUp\(F4\)\)", timeout=1))
        test_assert("Close FileBrowser reap", instance.wait_for_line(r"Reaping thread 0x[0-9a-f]+\(\d+ /sysroot/bin/filebrowser#1\)", timeout=2))
    ...
    if True:
        ...
        test_assert("Close FileViewer reap", instance.wait_for_line(r"Reaping thread 0x[0-9a-f]+\(\d+ /sysroot/bin/fileviewer#1\)", timeout=2))
    ...
    if True:
        ...
        _keypress(instance, 'ret', "Menu 2", idle=False)
        _startapp(instance, "/sysroot/bin/fileviewer", timeout=5)
        test_assert("FileViewer window render", instance.wait_for_line(r"\[gui::windows\] - L\d+: WindowGroup::redraw: \d+ 'File viewer'", timeout=5))
        test_assert("FileViewer idle timeout", instance.wait_for_idle(timeout=3))
