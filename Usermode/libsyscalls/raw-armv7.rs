	...
	macro_rules! syscall_a {
		($id:expr, $( $reg:tt = $val:expr),*) => {{
			...
			asm!("swi #0"
				: "={r0}" (rv_l), "={r1}" (rv_h)
				: "{r0}" ($id as usize) $(, $reg ($val as usize))*
				:
				: "volatile"
				);
